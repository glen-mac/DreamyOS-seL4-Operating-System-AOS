...
int proc_next_pid(seL4_Word *new_pid);
...
proc_create(seL4_CPtr fault_ep) { 
    ...
    if (err) {
        panic("PANIC: out of process ids - allocated too many or procs haven't been deleted");
    }

    /* create new proces */
    new_proc = malloc(sizeof(proc));
    if (new_proc == NULL) {
        panic("PANIC: failed allocating memory for new process struct.");
    }
