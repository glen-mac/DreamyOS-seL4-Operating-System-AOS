...
static inline void
_fill_table(const seL4_BootInfo *bi)
{
    int i;
...
static int
__ut_cmp(const void *_a, const void *_b)
{
...
static inline void
_reduce_table(int min_bits, int max_bits)
{
...
static inline void
_select_largest_contiguous(void)
{
    int i;
...
static int
ut_translate_device(seL4_Word addr, seL4_Untyped *ret_cptr, 
...
int
ut_translate(seL4_Word addr, seL4_Untyped *ret_cptr, seL4_Word *ret_offset)
{
    int start, end;
...
int
ut_table_init(const seL4_BootInfo *bi)
{
    if (!bi)
        return !0;
...
void
ut_find_memory(seL4_Word *low, seL4_Word *high)
{
    *low = _low;
    *high = _high;
}
